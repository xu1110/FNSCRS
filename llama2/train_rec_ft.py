...
def parse_args():
    ...
    parser.add_argument("--output_dir", type=str, default='finetune_redial', help="Where to store the final model.")
    
    parser.add_argument('--log_dir', type=str, default = 'finetune_redial')
    ...
    parser.add_argument("--peft_model", type=str, default='pretrain_redial')
    # optim
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if args.kl_coef != 0:
                logits = outputs['logits'][:, item_ids]
                with torch.no_grad():
                    logits_ref = ref_model.forward(**batch['input'])['logits'][:, item_ids]
                    logits_ref = logits_ref.detach()
                kl_lose = nn.KLDivLoss(reduction='batchmean')(F.log_softmax(logits, dim=1), F.softmax(logits_ref, dim=1))
...
if __name__ == '__main__':
    ...
    if args.dtype == 'fp32':
        base_model = LlamaForSequenceClassification.from_pretrained(args.model, num_labels = kg['num_entities'], entity_dim = args.entity_dim,device_map='auto',torch_dtype=torch.float32, token = "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")# , quantization_config = bnb_config
    elif args.dtype == 'fp16':
        base_model = LlamaForSequenceClassification.from_pretrained(args.model, num_labels = kg['num_entities'], entity_dim = args.entity_dim,device_map='auto',torch_dtype=torch.float16, token = "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")# , quantization_config = bnb_config
    elif args.dtype == 'bfp16':
        base_model = LlamaForSequenceClassification.from_pretrained(args.model, num_labels = kg['num_entities'], entity_dim = args.entity_dim,device_map='auto',torch_dtype=torch.bfloat16, token = "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")# , quantization_config = bnb_config
        
    logger.info(f'dtype: {args.dtype}')
    
    model = PeftModel.from_pretrained(base_model, args.peft_model, is_trainable=True)
    
    model.print_trainable_parameters()
    
    if args.kl_coef != 0:
        ref_model = PeftModel.from_pretrained(base_model, args.peft_model, is_trainable=False)

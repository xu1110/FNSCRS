...
if __name__ == '__main__':
    ...
    tokenizer = AutoTokenizer.from_pretrained(args.tokenizer, padding_side = args.pad_side)# 
    tokenizer.pad_token_id = 0


    if args.dtype == 'fp32':
        model = LlamaForSequenceClassification.from_pretrained(args.model, num_labels = kg['num_entities'], entity_dim = args.entity_dim,device_map='auto',torch_dtype=torch.float32)# , quantization_config = bnb_config
    elif args.dtype == 'fp16':
        model = LlamaForSequenceClassification.from_pretrained(args.model, num_labels = kg['num_entities'], entity_dim = args.entity_dim,device_map='auto',torch_dtype=torch.float16)# , quantization_config = bnb_config
    elif args.dtype == 'bfp16':
        model = LlamaForSequenceClassification.from_pretrained(args.model, num_labels = kg['num_entities'], entity_dim = args.entity_dim,device_map='auto',torch_dtype=torch.bfloat16)# , quantization_config = bnb_config
    ...
    train_data_aug_file = os.path.join(args.dataset, args.aug_dataset)
    train_data_aug_list = load_jsonl_data(train_data_aug_file)
    train_dataset_aug = DatasetForRec(
        data_list=train_data_aug_list, entity2id=kg['entity2id'],

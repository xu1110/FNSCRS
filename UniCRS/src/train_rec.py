...
def parse_args():
    ...
    parser.add_argument("--output_dir", type=str, default='/data/UniCRS/redial_rec_gritlm_data_aug_pre_50_not_removed', help="Where to store the final model.")
    parser.add_argument("--log_dir", type=str, default='redial_rec_gritlm_data_aug_pre_50_not_removed')
    ...
    parser.add_argument("--prompt_encoder", type=str, default='./redial_pre_new/best')
    # optim
...
if __name__ == '__main__':
    ...
    if args.kl_coef != 0:
        ...
        if args.prompt_encoder is not None:
            prompt_encoder_ref.load(args.prompt_encoder)
        prompt_encoder_ref = prompt_encoder_ref.to(device)
        prompt_encoder_ref.requires_grad_(False)
    ...
    train_dataset = CRSRecDataset(
        ...
        entity_max_length=args.entity_max_length, path=args.dataset_name,
    )
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if args.kl_coef == 0:
                ...
                model_output = model(**batch['context'], rec=True)
                logits = model_output.rec_logits
                loss = model_output.rec_loss
                with torch.no_grad():
                    prompt_embeds_ref = prompt_encoder_ref(
                        entity_ids=batch['entity'],
                        token_embeds=token_embeds,
                        output_entity=True,
                        use_rec_prefix=True,
                    )
                    batch['context']['prompt_embeds'] = prompt_embeds_ref
                    batch['context']['entity_embeds'] = prompt_encoder_ref.get_entity_embeds()
                    logits_ref =  model(**batch['context'], rec=True).rec_logits
